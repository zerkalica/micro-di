...
function MicroDiContainer(options) {
	...
	var selfInstanceId = this._config.get('microdi.setContext')['@instanceId'];
...
proto._clone = function _clone(contexts) {
	var resultContext = {};
	var name;
...
proto.setContext = function setContext(contextName, cb) {
	...
	var container = this._clone(contexts);
	cb(container);
