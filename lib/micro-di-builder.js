...
proto._extractProto = function _prepareCommand(key, val, obj, acc) {
	...
	if (!node['@instanceId']) {
		...
		for(i = 0, j = path.length; i < j; i++) {
			...
			if (currentScope) {
				assertOk(this._scopeDeps[currentScope] !== undefined, Exceptions.ScopeNotRegistered, {scope: currentScope});
				assertOk(
					!prevScope ||
...
proto.getContainer = function getContainer() {
	...
	var selfInstanceId = this._buildedConfig.get('microdi.setContext')['@instanceId'];
