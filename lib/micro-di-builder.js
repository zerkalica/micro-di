 ...
 */
proto.getContainer = function getContainer(parentContainer, scopes) {
	var instances = {};
	scopes = scopes || [];
	scopes = Array.isArray(scopes) ? scopes : [scopes];
	scopes.forEach(function copyScope(scope) {
		instances[scope] = parentContainer.getScope(scope);
	});

	var MicroDi = {
		getContainer: this.getContainer.bind(this),
		currentContainer: new MicroDiBuilder.Container({
			instances: instances,
			baseTypes: baseTypes
		})
	};
	...
	MicroDi.currentContainer._config = this._buildedConfig;

	return MicroDi.currentContainer;
};
