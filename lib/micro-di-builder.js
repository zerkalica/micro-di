...
function MicroDiBuilder(options) {
	...
	for (var scope in defs) {
		var def = defs[scope];
		this._scopeDefs[scope] = def.deps;
		...
		for(i = 0, j = path.length; i < j; i++) {
			...
			if (currentScope) {
				...
				assertThrow(
					...
					currentScope === this._scopeDefs[prevScope], // если текущий scope не числится в зависимостях prevScope - все ок
					// переход req->default допустим, default->req - нет
					// this._scopeDefs['req']: 'default', currentScope: 'default'
					// this._scopeDefs['default']: null, currentScope: 'req'
