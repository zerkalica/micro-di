...
function MicroDiBuilder(options) {
	...
	this._scopeDefs = options.scopeDefs || defaultScopeDefs;
}
...
proto._extractProto = function _prepareCommand(key, val, obj, acc) {
	...
	if (!node['@instanceId']) {
		...
		for(i = 0, j = path.length; i < j; i++) {
			...
			if (currentScope) {
				assertThrow(this._scopeDefs[currentScope] !== undefined, Exceptions.ScopeNotRegistered, {scope: currentScope});
				assertThrow(
					!prevScope ||
					prevScope === currentScope || // если scope не изменился - все ок
					currentScope === this._scopeDefs[prevScope].deps, // если текущий scope не числится в зависимостях prevScope - все ок
					// переход req->default допустим, default->req - нет
					// this._scopeDefs['req'].deps: 'default', currentScope: 'default'
					// this._scopeDefs['default'].deps: null, currentScope: 'req'
...
proto._getScopeFromTags = function (tags) {
	for (var scope in this._scopeDefs) {
		var defTags = this._scopeDefs[scope].tags;
		for (var i = 0; i < tags.length; i++) {
			if (defTags.indexOf(tags[i]) !== -1) {
				return scope;
			}
		}
	}
...
proto.getContainer = function getContainer() {
	function cloneContainer(scopes) {
		return this._cloneContainer(cloneContainer.container, scopes);
	}
	cloneContainer.container = null;

	if (!this._buildedConfig) {
		var MicroDi = {
			cloneContainer: cloneContainer.bind(this)
		};
		this.addConfig({MicroDi: MicroDi});
		this._buildedConfig = this._config.build();
	}

	var container = this._cloneContainer();
	cloneContainer.container = container;

	return container;
};

proto._cloneContainer = function _cloneContainer(container, scopes) {

	var instances = {};
	scopes = scopes || [];
	scopes = Array.isArray(scopes) ? scopes : [scopes];
	scopes.forEach(function copyScope(scope) {
		instances[scope] = container.getScope(scope);
	});
